...
def parse_answers(file_path: str, validate: bool = True) -> dict:
    ...
    if len(answers) < 100:  # If we didn't find enough answers, try alternative patterns
        ...
        print(
            f"  Found {len(alt_matches)} additional answers using alternative pattern"
        )
...
def generate_statistics(exam_data: Dict) -> Dict:
    ...
    stats = {
        "total_questions": len(questions),
        "avg_question_length": (
            sum(len(q["question"]) for q in questions) / len(questions)
            if questions
            else 0
        ),
        "avg_option_length": (
        ...
        "questions_with_short_text": sum(
            1 for q in questions if len(q["question"]) < 50
        ),
        "questions_with_long_text": sum(
            1 for q in questions if len(q["question"]) > 500
        ),
    }
    return stats
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--validate", action="store_true", default=True, help="Validate parsed data"
    )
        ...
        for exam_type, files in types.items():
            ...
            if "questions_pdf" in files:
                ...
                parsed_questions = parse_questions(
                    questions_pdf_path, validate=args.validate
                )
                print(
