...
def upload_exam_data_to_firestore(year):
    ...
    try:
        ...
        for exam_type in exam_types:
            ...
            collection_ref = db.collection('data').document('exams').collection(f'{exam_type}_{year}')
            ...
            for i in range(0, len(exam_data), batch_size):
                ...
                # Commit the batch
                batch.commit()
                print(f"  Uploaded batch {i//batch_size + 1}: {len(batch_data)} documents")

            print(f"Successfully uploaded {uploaded_count} documents to collection: data/exams/{exam_type}_{year}")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        print("This will upload all three exam types: adwokacki_radcowy, komorniczy, notarialny")
        sys.exit(1)
