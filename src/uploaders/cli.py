...
def upload(
    ...
    model: Annotated[
        Optional[str],
        typer.Option(
            "--model", "-m", help="Upload only a specific model (by ID or name)"
        ),
    ] = None,
    dry_run: Annotated[
        bool,
    ...
    if model:
        filtered = [
            m
            for m in models
            if model.lower() in m.model_id.lower()
            or model.lower() in m.model_name.lower()
        ]
        if not filtered:
...
def list_models(
    path: Annotated[
        Path,
        typer.Option("--path", "-p", help="Path to the directory containing results"),
    ] = DEFAULT_PATH,
):
    """
